...
- **Proximity alerts.** Depth Anything V2 (metric, ViT-S/Hypersim) runs on the same frames; if anything within ~50 cm is detected, a warning beep plays. A one-time calibration step turns the user's height into a per-device scale factor.
- **Turn-by-turn navigation.** Given a destination phrase, the server resolves it to coordinates via Naver Search + Naver Cloud Maps geocoding, fetches a walking route, and advances through waypoints as the browser's GPS reports new positions.

## Architecture

```
Browser (templates/index.html, static/script.js)
  │  camera frames, GPS, TTS/STT
  ▼
Flask server (server.py) ──► Gemini (vision)         scene description
                          ├► Depth Anything V2       proximity warning
                          └► Naver Search + NCP Maps geocoding + directions
```

| Layer | File / Module |
| --- | --- |
| HTTP + orchestration | `server.py` |
| Depth model | `depth_anything_v2/` (DINOv2 backbone + DPT head) |
...
- `Space` — start / stop scene auto-analysis
...
| POST | `/describe`                  | Submit a frame, get description (auto-starts pipelining) |
...
| GET  | `/get_queue_status`         | Diagnostic: pipeline / API / TTS state |
| POST | `/calibrate`                 | One-shot calibration from height + center-frame depth |
| POST | `/analyze_depth`             | Detect objects within ~50 cm using the calibration factor |
...
| GET  | `/logs`, `/logs/clear`       | Tail / clear `server.log` |

## How calibration works

The user enters their height in cm. The server estimates arm length as `height * 0.26 / 100` meters (e.g., 175 cm → ~0.45 m), takes a depth reading at the center of a frame held at arm's length, and stores `arm_length / measured_depth` as a per-user scale factor. Subsequent depth maps are multiplied by that factor before the 0.5 m proximity threshold is applied.

## Notes

- The pipelining design optimizes for *time-to-spoken-result*: a new request can return a previously-queued description instantly while the next one is already in flight on a different API key.
...
- Only the V1 (key-id / key) Naver Cloud Maps Directions endpoint is wired up. Migration to the API-Gateway v2 auth scheme is a future change.
