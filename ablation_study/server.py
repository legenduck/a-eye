...
def get_image_list():
    try:
        ...
        logger.info(f"Image file list found: {len(image_names)} files")
        return jsonify({"images": image_names})
        
    except Exception as e:
        logger.error(f"Image list query error: {str(e)}")
...
def performance_report():
    if not performance_data:
        return jsonify({"error": "Performance data not found"}), 400
