...
# --- Global state ---
navigation_sessions = {}
latest_response = None  # latest response only (pipelining)
...
for i in range(1, 4):  # API_KEY_1, API_KEY_2, API_KEY_3
    ...
    if key:
        api_keys.append(key)
        logger.info(f"API_KEY_{i} loaded")

if not api_keys:
    single_key = os.getenv("API_KEY")
    if single_key:
        api_keys.append(single_key)
        logger.info("Default API_KEY loaded")
    else:
        raise ValueError("At least one Gemini API key is required (API_KEY_1, API_KEY_2, API_KEY_3 or API_KEY)")

logger.info(f"Total {len(api_keys)} Gemini API keys available")
...
SUPPORTED_MODELS = {
    'gemini-2.0-flash': {
        'type': 'gemini',
        'name': f'Gemini 2.0 Flash (parallel x{len(api_keys)})',
        'model_name': 'gemini-2.0-flash'
    },
    'gemini-1.5-flash': {
        'type': 'gemini',
        'name': f'Gemini 1.5 Flash (parallel x{len(api_keys)})',
        'model_name': 'gemini-1.5-flash'
    },
    'gemini-1.5-flash-8b': {
        'type': 'gemini',
        'name': f'Gemini 1.5 Flash 8B (parallel x{len(api_keys)})',
        'model_name': 'gemini-1.5-flash-8b'
    },
    'gemini-2.0-flash-lite': {
        'type': 'gemini',
        'name': f'Gemini 2.0 Flash Lite (parallel x{len(api_keys)})',
        'model_name': 'gemini-2.0-flash-lite'
    },
    'gemini-1.5-pro': {
        'type': 'gemini',
        'name': f'Gemini 1.5 Pro (parallel x{len(api_keys)})',
...
def analyze_image_single(image_pil, api_idx, model_name='gemini-2.0-flash'):
    prompt_parts = [
        """You are a professional AI assistant helping visually impaired people walk safely.

Provide information according to the following priority:

1. **Immediate hazards** (top priority):
   - Obstacles directly ahead (people, pillars, construction zones, vehicles, etc.)
   - Stairs, slopes, potholes
   - Traffic light status, crosswalk situation
   - Open or closed doors

2. **Direction and movement information**:
   - Forks, intersection directions
   - Door positions and entrance information
   - Elevator and escalator locations

3. **Important text information**:
   - Bus numbers, subway lines
   - Store names, building names
   - Important sign content (restrooms, exits, floor numbers, etc.)

**Information to exclude**:
- Colors, designs, decorative elements
- Clothing or appearance of people
- Detailed background descriptions
- Unnecessary details such as contents inside containers

**Response format**:
- Include distance ("2 meters ahead", "directly ahead", "to the left")
- Concise action instructions ("detour", "you may proceed straight")
- 1-2 sentences, only the essentials
- Do not use unnecessary phrases like "in the photo", "in the image"

Examples:
- "There is a pillar 1 meter directly ahead, detour to the left."
- "The crosswalk traffic light is red. Please wait."
- "Bus stop number 7 is on the left."
...
Now analyze the image:""",
    ...
    try:
        ...
        logger.info(f"API {api_idx} responded in {processing_time:.3f}s")
        ...
        logger.error(f"API {api_idx} call failed: {e}")
        return {
...
def process_api_response(request_id, api_idx, result):
    ...
    if stop_event.is_set():
        logger.info(f"Discarded API {api_idx} response - system stopped (request ID: {request_id[:8]})")
        return

    if tts_status["is_speaking"]:
        logger.info(f"Discarded API {api_idx} response - TTS in progress (request ID: {request_id[:8]})")
        return

    if not result["success"]:
        logger.warning(f"Discarded API {api_idx} response - call failed (request ID: {request_id[:8]})")
        return
    ...
    with response_lock:
        ...
        logger.info(f"Updated latest response from API {api_idx} (request ID: {request_id[:8]})")

def continuous_processing_worker():
    logger.info("Auto image processing worker started (Event-based)")
    ...
    while not stop_event.is_set():
        try:
            ...
            logger.info(f"Starting image analysis on API {current_api_idx} (request ID: {request_id[:8]})")
            ...
            def api_call_worker():
                if stop_event.is_set():
                    logger.info(f"API {current_api_idx} call cancelled")
                    if request_id in pending_requests:
                        del pending_requests[request_id]
                    return
            ...
            if stop_event.wait(timeout=1.0):
                logger.info("Stop signal detected - exiting worker loop")
                break

        except Exception as e:
            logger.error(f"Auto processing worker error: {e}")
            if stop_event.wait(timeout=1.0):
                break

    logger.info("Auto image processing worker stopped")
...
def upload_image():
    ...
    try:
        ...
        logger.info(f"New image registered - size: {image_pil.size}")

        return jsonify({
            "message": "Image registered. Auto analysis will start.",
...
def start_auto_processing():
    if auto_processing["enabled"]:
        return jsonify({"message": "Auto processing is already running"})

    start_auto_processing_worker()

    logger.info("Auto image processing started")
    return jsonify({"message": "Auto image processing started"})
...
def stop_auto_processing():
    ...
    if auto_processing["thread"] and auto_processing["thread"].is_alive():
        ...
        if auto_processing["thread"].is_alive():
            logger.warning("Background thread did not terminate within 2 seconds")
        else:
            logger.info("Background thread terminated normally")

    auto_processing["thread"] = None

    logger.info("Pipelining system fully stopped (Event-based)")

    return jsonify({
        "message": "Pipelining system has been fully stopped",
        "method": "threading.Event based immediate stop",
        "cleared": {
            "stop_event": True,
            "current_image": True,
...
def get_response():
    ...
    with response_lock:
        if latest_response is None:
            return jsonify({"message": "No new analysis result available"}), 204

        response = latest_response.copy()
        latest_response = None

        logger.info(f"Returning latest response: {response['description'][:50]}...")
...
def set_tts_status():
    ...
    status = "started" if is_speaking else "completed"
    logger.info(f"TTS status updated: {status} - '{current_text[:30]}...'")

    return jsonify({"message": f"TTS status updated to '{status}'"})
...
# --- Legacy describe endpoint (kept for compatibility, with parallel processing) ---
...
def describe():
    ...
    logger.info(f"=== Image upload and pipelining started - model: {model_id} ===")

    if 'image' not in request.files:
        logger.warning("Request did not include an image file")
        return jsonify({"error": "No image file provided"}), 400
    ...
    try:
        ...
        # Decouple from the request file stream by round-tripping through numpy
        ...
        if not auto_processing["enabled"]:
            start_auto_processing_worker()
            logger.info("Pipelining auto-started")

        with response_lock:
            if latest_response is not None:
                response = latest_response.copy()
                latest_response = None
                logger.info(f"Returning existing response immediately: {response['description'][:50]}...")
                return jsonify({
                    "description": response["description"],
                    "model_name": f"Gemini 2.0 Flash (API {response['api_idx']}) - pipelining",
                    "processing_time": time.time() - request_start,
                    "pipelining": True
                })

        # Wait up to 3 seconds for a new response
        max_wait_time = 3.0
        ...
        while (time.time() - wait_start) < max_wait_time:
            ...
            with response_lock:
                if latest_response is not None:
                    ...
                    logger.info(f"Returning new response (waited {total_time:.3f}s): {response['description'][:50]}...")
        ...
        total_time = time.time() - request_start
        logger.warning(f"Pipelining response timed out ({total_time:.3f}s)")

        return jsonify({
            "description": "Analysis in progress. Please try again shortly.",
            "model_name": "Gemini 2.0 Flash - pipelining (timeout)",
            "processing_time": total_time,
            "pipelining": True,
            "timeout": True
        })
        ...
        logger.error(f"Image processing error - time: {error_time:.3f}s, error: {str(e)}")
...
def navigation_describe():
    ...
    try:
        ...
        prompt = f"""You are a professional AI assistant providing navigation guidance for visually impaired people.

**Current navigation context**:
- Destination: {navigation_info['goal_query']}
- Current instruction: {navigation_info['current_instruction']}
- Progress: {navigation_info['instruction_index'] + 1}/{navigation_info['total_instructions']}

**Provide information by priority**:

1. **Immediate hazards related to the route** (top priority):
   - Obstacles in the direction of travel (people, pillars, construction zones, vehicles, etc.)
   - Stairs, slopes, potholes
   - Traffic light status, crosswalk situation
   - State of doors or entrances along the route

2. **Direction confirmation**:
   - Whether the current instruction matches the actual environment
   - Correct turn at forks and intersections
   - Confirmation of arriving at building entrances or specific points

3. **Additional information for safe travel**:
   - Bus numbers, subway lines (when using public transit)
   - Store names, building names (for location confirmation)
   - Important sign content

**Response format**:
- Specific guidance prioritizing the navigation direction
- Include distance and direction ("2 meters ahead", "to the right")
- State whether the current instruction is executable
- 1-2 sentences, only the essentials

Examples:
- "Go straight as instructed. The walkway directly ahead is clear."
- "Turn-left point. There is a crosswalk on the left, please wait for the signal."
- "The destination building entrance is 3 meters ahead on the right."

Now analyze the image and provide information helpful for navigation:"""
...
def try_ncp_v1_request(start, goal_query):
    try:
        ...
        final_instruction = f"Ending route guidance. Total distance {total_dist:.1f} kilometers, estimated time about {total_dura:.0f} minutes."
...
try:
    ...
    logger.error("Checkpoint file not found. Make sure a valid checkpoint file exists in 'checkpoints/'.")
    depth_model = None
except Exception as e:
...
def calibrate():
    if DEVICE == 'cpu':
        return jsonify({"error": "Only available in GPU environment"}), 400
    ...
    try:
        # Estimate arm length (m) from height (cm). Empirical: 175cm -> 45cm.
        ...
        # Use the depth at the screen center as the measurement
        h, w = depth_map.shape
        ...
        logger.info(f"Calibration completed: height({user_height_cm}cm), estimated arm length({estimated_arm_length_m:.3f}m), measured depth({measured_depth:.3f}), factor({calibration_factor:.3f})")
...
def analyze_depth():
    try:
        ...
        if depth_map is None:
            return jsonify({"error": "Depth analysis failed"}), 500

        depth_map_calibrated = depth_map * calibration_factor

        # Detect obstacles within 50cm
        ...
        # Closest distance in the central region
        ...
        # Warn if center has an object within 50cm or >10% of the frame is an obstacle
        should_warn = min_distance < obstacle_threshold or obstacle_ratio > 0.1

        global_min_distance = float(np.min(depth_map_calibrated))

        if should_warn:
            logger.warning(f"Obstacle detected! Center min: {min_distance:.2f}m, global min: {global_min_distance:.2f}m, obstacle ratio: {obstacle_ratio:.1%}")
        else:
            logger.info(f"Safe - center min: {min_distance:.2f}m, global min: {global_min_distance:.2f}m, obstacle ratio: {obstacle_ratio:.1%}")

        return jsonify({
            "should_warn": bool(should_warn),
            "min_distance": round(float(min_distance), 2),
            "obstacle_ratio": round(float(obstacle_ratio), 3),
            "message": f"Closest object: {min_distance:.2f}m" + (" - warning!" if should_warn else "")
        })

    except Exception as e:
        logger.error(f"Error during depth analysis: {e}", exc_info=True)
        return jsonify({"error": "A server error occurred during depth analysis."}), 500
